...
def build_queue_embed(items: list[Track], *, page: int, page_size: int, max_page: int) -> discord.Embed:
    if not items:
        embed = discord.Embed(title='Очередь пуста', color=discord.Color.blurple())
        embed.set_footer(text='Страница 1 / 1')
        return embed
    start = page * page_size
    end = min(start + page_size, len(items))
    lines = [
        f'{i + 1}. [{format_track_label(t)}]({t.url})' if t.url
        else f'{i + 1}. {format_track_label(t)}'
